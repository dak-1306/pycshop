...
class Seller {
  ...
  static async getShopInfo(sellerId) {
    try {
      const query = `
        SELECT 
    c.ID_CuaHang,
    ...
    c.NgayCapNhat,
    d.TenDanhMuc,
    n.HoTen AS OwnerName,
    n.Email AS OwnerEmail
FROM CuaHang c
...
WHERE n.ID_NguoiDung = ?;
      ...
      console.error("[SELLER] Error in getShopInfo:", error);
      throw error;
    }
  }

  // Update seller shop information
  static async updateShopInfo(sellerId, shopData) {
    try {
      const { tenCuaHang, danhMuc, diaChi, soDienThoai } = shopData;
...
WHERE n.ID_NguoiDung = ?;
      ...
      console.error("[SELLER] Error in updateShopInfo:", error);
      throw error;
    }
  }

  // Create new shop for user becoming seller
  static async createShop(sellerId, shopData) {
    try {
      const { tenCuaHang, danhMuc, diaChi, soDienThoai } = shopData;

      // Step 1: Insert new shop into cuahang table
      const insertShopQuery = `
        INSERT INTO cuahang (TenCuaHang, ID_DanhMuc, DiaChiCH, SoDienThoaiCH, NgayCapNhat)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
      `;

      const [shopResult] = await db.execute(insertShopQuery, [
        tenCuaHang,
        danhMuc,
        diaChi,
        soDienThoai,
      ...
      // Step 2: Update nguoidung table to link with the new shop
      ...
      await db.execute(updateUserQuery, [shopId, sellerId]);

      return shopId;
    } catch (error) {
      console.error("[SELLER] Error in createShop:", error);
      throw error;
    }
  }
