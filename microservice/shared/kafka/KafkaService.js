...
class KafkaService {
  ...
  async createConsumer(groupId, topics = []) {
    try {
      ...
      console.error(`[KAFKA] Failed to create consumer for group ${groupId}:`, error);
      throw error;
    }
  }
  ...
  async sendOrderEvent(eventType, orderData) {
    ...
    return await this.sendMessage(topic, message, orderData.orderId?.toString());
  }
  ...
  async sendNotificationEvent(eventType, notificationData) {
    ...
    return await this.sendMessage(topic, message, notificationData.userId?.toString());
  }
