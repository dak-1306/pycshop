    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      {statsConfig.map((config) => {
        ...
        return (
          <div
            key={config.key}
            className={`${config.bgColor} rounded-xl p-6 border ${config.borderColor}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className={`${config.textColor} text-sm font-medium mb-1`}>
                  {config.title}
                </p>
                <p className={`text-3xl font-bold ${config.valueColor}`}>
                  {statData.formattedValue}
                  <span className={`${config.unitColor} text-xs`}>
                    {config.unit}
                  </span>
                </p>
                <p className={`text-sm font-medium ${statData.changeColor}`}>
                  {statData.changeIcon} {Math.abs(statData.change)}% so với
                  tháng trước
                </p>
              </div>
              <div
